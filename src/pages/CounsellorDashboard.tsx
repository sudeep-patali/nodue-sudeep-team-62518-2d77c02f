...
import ApplicationDetailModal from "@/components/counsellor/ApplicationDetailModal";
...
export default function CounsellorDashboard() {
  ...
  const pendingApplications = applications.filter(
    (app) => !app.counsellor_verified && app.status !== "rejected"
  );
  const approvedApplications = applications.filter(
    (app) => app.counsellor_verified && app.status !== "rejected"
  );
  const rejectedApplications = applications.filter(
    (app) => app.status === "rejected" && app.counsellor_comment
  );
  ...
  const ApplicationCard = ({ application }: { application: any }) => {
    ...
    const isRejected = application.status === "rejected" && application.counsellor_comment;
    const isApproved = application.counsellor_verified;
  ...
  return (
    <DashboardLayout role="counsellor" title="Counsellor Dashboard">
